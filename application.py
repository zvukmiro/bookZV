...
# after search, if you click on particular book, the details are displayed and you can leave review
...
def book(isbn):
    ...
    try:
        ...
        reviews = db.execute("SELECT ROUND(AVG(rating), 1) FROM reviews WHERE rw_isbn=:isbn", {"isbn":isbn})
        if reviews.rowcount==1:
            review = reviews.first()[0]
        res =  requests.get("https://www.goodreads.com/book/review_counts.json", params={"key": "SlE9tBsqoV9Pzyb7ENbHXg", "isbns": isbn})
        review2=(res.json())['books'][0]['average_rating']
        return render_template("book.html", book=book, review=review, review2=review2)
    finally:
        db.close()

# without review function here and in book.html it works
...
def review():
    ...
    #message="Your review did not post."
    return redirect(url_for('error'))
